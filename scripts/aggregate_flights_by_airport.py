...
for row in reader:
    ...
    #calculate min
    min = airlines[unique_carrier, airport, month][2] if airlines[unique_carrier, airport, month][2] != 0 else sys.maxsize
    airlines[unique_carrier, airport, month][2] = delayed_hours if min is None else delayed_hours if delayed_hours < min and delayed_hours > 0 else min
    # calculate max
    ...
    if(cancellation_code == "A"): # late aircraft delay
        airlines[unique_carrier, airport, month][41] += 1
    
    if(cancellation_code == "B"): # weather delay
        airlines[unique_carrier, airport, month][42] +=1
    
    if(cancellation_code == "C"): # nas delay
        airlines[unique_carrier, airport, month][43] +=1
    
    if(cancellation_code == "D"): # security delay
        airlines[unique_carrier, airport, month][44] +=1
    
    airlines[unique_carrier, airport, month][45] += 1 # count total flights
